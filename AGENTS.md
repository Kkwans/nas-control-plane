...
- OpenSpec 改为按需使用，不再作为每个功能切片的默认流程。简单修复、明确需求和局部功能直接按本文件与当前任务实现。
- 中大型功能先维护一份精炼的长期方案和阶段任务；重大架构调整、跨模块协议、数据迁移、备份恢复或高风险操作确有长期追溯价值时，再使用 OpenSpec 或 ADR。
- 不为流程生成重复、低价值文档，也不要求每个小功能执行 proposal、specs、tasks 和 archive 全流程。
- 已有 OpenSpec 文件继续保留；处于活动状态的变更可作为对应阶段记录，但不阻塞后续明确实现。
