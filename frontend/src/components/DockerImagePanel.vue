...
const filteredDownloadJobs = computed(() => {
  ...
  return downloadJobs.value.filter((job) =>
    (downloadStatus.value === 'all' || job.status === downloadStatus.value) &&
  ...
  <section class="image-workspace">
    ...
    <section v-else class="download-workspace panel">
      ...
      <article v-for="job in pagedDownloadJobs" :key="job.id" class="download-row">
        ...
        <button v-else-if="job.status === 'completed'" type="button" @click="showDownloadedImage(job)">查看镜像</button>
        <span v-else class="job-live">实时更新</span>
      </article>
...
.mode-actions{width:min(460px,45vw)}.mode-actions :deep(.el-input){flex:1}.download-mode-actions{width:min(620px,58vw)}.download-mode-actions .ncp-select{width:150px}
...
.hub-workspace{height:clamp(520px,calc(100dvh - 304px),650px);min-height:0;overflow:hidden}.repository-list{height:100%;max-height:none;border-radius:14px;background:#fff}.repository-list>button{margin:4px 6px 0;border:1px solid transparent;border-radius:10px;background:#fff}.repository-list>button:hover{border-color:var(--ncp-line);background:var(--ncp-surface-hover)}.repository-list>button.active{border-color:rgba(52,116,212,.34);background:linear-gradient(90deg,rgba(234,242,253,.9),rgba(248,250,253,.9));box-shadow:0 5px 16px rgba(42,83,140,.08)}.repo-icon img{width:24px;height:24px;object-fit:contain}.repo-icon b{font-size:.9rem}
.repository-detail{height:100%;min-height:0;overflow:hidden}.repository-detail>p{min-height:0;margin:12px 0;line-height:1.55}.repository-facts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));margin:0 0 12px;border:1px solid var(--ncp-line);border-radius:10px;background:var(--ncp-surface-quiet)}.repository-facts>div{display:grid;min-width:0;gap:3px;padding:9px 11px}.repository-facts>div:nth-child(n+4){border-top:1px solid var(--ncp-line)}.repository-facts>div:not(:nth-child(3n+1)){border-left:1px solid var(--ncp-line)}.repository-facts dt{color:var(--ncp-text-subtle);font-size:.68rem}.repository-facts dd{overflow:hidden;margin:0;color:var(--ncp-text);font-size:.76rem;font-weight:650;text-overflow:ellipsis;white-space:nowrap}.tag-picker{min-height:0}.tag-picker .ncp-select{width:100%}.tag-picker dl{background:#fff}.repository-detail>footer{padding-top:12px}.repository-detail>footer .primary-button{flex:0 0 auto}
.download-head>:nth-child(n+2),.download-row>:nth-child(n+2){justify-self:center;text-align:center}.download-row .job-state{justify-self:center}.download-row>button{justify-self:center}.job-live{color:var(--ncp-success-strong);font-size:.72rem;font-weight:680}
