  ...
  <div v-if="table && source" class="page workspace-page table-workbench-page">
    ...
    <section v-if="mode === 'data'" class="data-workbench panel">
      ...
      <div class="data-table">
        <ElTable v-loading="rowsLoading" :data="tableRows?.rows ?? []" height="100%" border stripe highlight-current-row @sort-change="onSortChange">
    ...
    <section v-else-if="mode === 'overview'" class="info-workbench">
      <div class="table-profile panel">
        ...
        <div><span>对象类型</span><strong>{{ table.type }}</strong></div>
        <div><span>创建时间</span><strong>{{ table.createdAt || '数据库未提供' }}</strong></div>
      ...
      <article class="panel table-description"><h2>表信息</h2><p>该页面汇总数据库返回的结构与容量信息。SQLite 不记录单表创建时间；MySQL / MariaDB 可读取 information_schema 中的创建时间和大小。</p></article>
    </section>
    ...
    <section v-else-if="mode === 'definition'" class="definition-workbench panel">
      ...
      <pre v-if="table.definition"><code>{{ table.definition }}</code></pre>
...
.info-workbench{display:grid;grid-template-columns:1.5fr 1fr;gap:12px}.table-profile{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1px;padding:1px;overflow:hidden;background:var(--ncp-line)}.table-profile>div{display:grid;gap:5px;min-height:112px;align-content:center;padding:18px;background:#fff}.table-profile span{color:var(--ncp-text-subtle);font-size:.75rem}.table-profile strong{overflow:hidden;font-size:.88rem;text-overflow:ellipsis;white-space:nowrap}.table-description{padding:20px}.table-description h2{margin:0 0 8px;font-size:1rem}.table-description p{margin:0;color:var(--ncp-text-muted);font-size:.82rem;line-height:1.75}
.structure-workbench :deep(.el-table){font-size:.8rem}.structure-name{display:flex;align-items:center;gap:5px;font-weight:700}
.definition-workbench{display:flex;flex-direction:column}.definition-workbench header{display:flex;min-height:60px;align-items:center;padding:0 16px;border-bottom:1px solid var(--ncp-line)}.definition-workbench header>div{display:flex;align-items:center;gap:9px}.definition-workbench header span{display:grid}.definition-workbench header strong{font-size:.84rem}.definition-workbench header small{color:var(--ncp-text-subtle);font-size:.72rem}.definition-workbench pre{min-height:0;flex:1;margin:0;padding:20px;overflow:auto;background:#182235;color:#dce8fa;font-family:'JetBrains Mono Variable',monospace;font-size:.78rem;line-height:1.75;white-space:pre-wrap}.definition-empty{display:grid;min-height:300px;place-content:center;padding:24px;color:var(--ncp-text-subtle);font-size:.82rem;text-align:center}
