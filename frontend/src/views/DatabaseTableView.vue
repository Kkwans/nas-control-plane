...
const databaseErrorCopy: Record<string, string> = {
  credentials_required: '连接需要数据库凭据，请返回详情页完成连接。',
  auth_failed: '数据库认证失败，请返回详情页核对用户名和密码。',
  unreachable: '数据库服务不可达，请检查连接状态。',
  ...
  credential_corrupt: '保存的数据库凭据不可用，请返回详情页重新连接。',
...
function toErrorState(error: unknown, fallback: string): ErrorState {
  return { code: errorCode(error), message: errorMessage(error, fallback) }
}
...
<template>
  <div v-if="table && source" class="page workspace-page table-workbench-page">
    <WorkspaceHeader :title="table.name" :description="`${tableTypeLabel(table.type)} / ${source.name}${table.schema ? ` / ${table.schema}` : ''}`" :icon="Table2" :stats="stats">
      <template #actions>
    ...
    <div v-if="errorState" class="database-error" role="alert">
      <CircleAlert :size="18" />
      <div><strong>数据表操作失败</strong><span>{{ errorState.message }}</span></div>
    ...
    <section v-else class="sql-workbench panel">
      ...
      <div class="query-result">
        ...
        <div v-if="queryError" class="query-message query-message--error"><CircleAlert :size="22" /><strong>执行失败</strong><code>代码 {{ queryError.code }}</code><span>{{ queryError.message }}</span></div>
