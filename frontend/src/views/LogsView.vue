  ...
  <div class="page workspace-page logs-page">
    ...
    <section class="log-toolbar panel">
      <div class="log-filters">
        <div class="log-filter-field"><span>日志来源</span><ElSelect v-model="source" aria-label="日志来源">
        ...
        <div v-if="source === 'container'" class="log-filter-field"><span>容器</span><ElSelect v-model="containerId" filterable placeholder="选择容器" aria-label="Docker 容器">
          <ElOption v-for="container in containers" :key="container.id" :label="container.name" :value="container.id" />
        </ElSelect></div>
        <div class="log-filter-field"><span>级别</span><ElSelect v-model="level" aria-label="日志级别"><ElOption label="全部级别" value="all" /><ElOption label="错误" value="error" /><ElOption label="警告" value="warning" /><ElOption label="信息" value="info" /><ElOption label="调试" value="debug" /></ElSelect></div>
        <div class="log-filter-field"><span>时间范围</span><ElSelect v-model="hours" aria-label="时间范围"><ElOption label="最近 1 小时" :value="1" /><ElOption label="最近 6 小时" :value="6" /><ElOption label="最近 24 小时" :value="24" /><ElOption label="最近 7 天" :value="168" /></ElSelect></div>
      </div>
      <div class="log-tools">
