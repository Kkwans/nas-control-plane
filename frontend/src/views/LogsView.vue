<script setup lang="ts">
import { computed, onBeforeUnmount, onMounted, ref, watch } from 'vue'
import { CirclePause, FileClock, Play, RefreshCw, Search } from '@lucide/vue'
import { ElButton, ElInput, ElOption, ElSelect, ElSwitch } from 'element-plus'

import { requestLogs, type LogEntry } from '@/api/control'
import WorkspaceHeader, { type WorkspaceStat } from '@/components/WorkspaceHeader.vue'
import { useSystemStore } from '@/stores/system'

type LogSource = 'system' | 'agent' | 'container'
const systemStore = useSystemStore()
const source = ref<LogSource>('system')
const containerId = ref('')
const level = ref('all')
const hours = ref(6)
const query = ref('')
const following = ref(false)
const loading = ref(false)
const error = ref('')
const entries = ref<LogEntry[]>([])
let followTimer: number | null = null

const containers = computed(() => systemStore.inventory?.containers ?? [])
const stats = computed<WorkspaceStat[]>(() => [
  { label: '当前记录', value: entries.value.length },
  { label: '错误', value: entries.value.filter((item) => item.level === 'error').length, tone: 'warning' },
  { label: '实时跟随', value: following.value ? '已开启' : '已暂停', tone: following.value ? 'success' : undefined },
])

async function load(silent = false) {
  if (source.value === 'container' && !containerId.value) {
    entries.value = []
    return
  }
  if (!silent) loading.value = true
  error.value = ''
  try {
    const result = await requestLogs({
      source: source.value, containerId: containerId.value, level: level.value,
      query: query.value, hours: hours.value, limit: 200,
    })
    entries.value = result.entries
  } catch {
    error.value = '日志读取失败，请确认目标服务正在运行。'
  } finally {
    loading.value = false
  }
}

function syncFollowTimer() {
  if (followTimer !== null) window.clearInterval(followTimer)
  followTimer = following.value ? window.setInterval(() => void load(true), 3000) : null
}

watch([source, containerId, level, hours], () => void load())
watch(following, syncFollowTimer)
onMounted(() => void load())
onBeforeUnmount(() => { if (followTimer !== null) window.clearInterval(followTimer) })

function levelLabel(value: LogEntry['level']) {
  return value === 'error' ? '错误' : value === 'warning' ? '警告' : value === 'debug' ? '调试' : '信息'
}
</script>

<template>
  <div class="page workspace-page logs-page">
    <WorkspaceHeader title="日志中心" description="统一检索系统、NCP Agent 与 Docker 容器日志" :icon="FileClock" :stats="stats">
      <template #actions><ElButton :loading="loading" @click="load()"><RefreshCw :size="16" />刷新</ElButton></template>
    </WorkspaceHeader>

    <section class="log-toolbar panel">
      <div class="log-filters">
        <ElSelect v-model="source" aria-label="日志来源">
          <ElOption label="系统 journald" value="system" /><ElOption label="NCP Root Agent" value="agent" /><ElOption label="Docker 容器" value="container" />
        </ElSelect>
        <ElSelect v-if="source === 'container'" v-model="containerId" filterable placeholder="选择容器" aria-label="Docker 容器">
          <ElOption v-for="container in containers" :key="container.id" :label="container.name" :value="container.id" />
        </ElSelect>
        <ElSelect v-model="level" aria-label="日志级别"><ElOption label="全部级别" value="all" /><ElOption label="错误" value="error" /><ElOption label="警告" value="warning" /><ElOption label="信息" value="info" /><ElOption label="调试" value="debug" /></ElSelect>
        <ElSelect v-model="hours" aria-label="时间范围"><ElOption label="最近 1 小时" :value="1" /><ElOption label="最近 6 小时" :value="6" /><ElOption label="最近 24 小时" :value="24" /><ElOption label="最近 7 天" :value="168" /></ElSelect>
      </div>
      <div class="log-tools">
        <ElInput v-model="query" clearable placeholder="搜索消息或服务" @keyup.enter="load()"><template #prefix><Search :size="16" /></template></ElInput>
        <label class="follow-switch"><component :is="following ? Play : CirclePause" :size="16" /><span>实时跟随</span><ElSwitch v-model="following" /></label>
      </div>
    </section>

    <div v-if="error" class="log-error">{{ error }}</div>
    <section class="log-console panel">
      <div class="log-head"><span>时间</span><span>级别</span><span>来源</span><span>消息</span></div>
      <template v-if="loading && !entries.length">
        <div v-for="row in 12" :key="row" class="log-row log-row--skeleton"><i v-for="cell in 4" :key="cell" class="ncp-skeleton"></i></div>
      </template>
      <div v-for="entry in entries" v-else :key="entry.id" class="log-row">
        <time>{{ new Date(entry.timestamp).toLocaleString('zh-CN', { hour12: false }) }}</time>
        <span :class="['level-badge', `level-badge--${entry.level}`]">{{ levelLabel(entry.level) }}</span>
        <code :title="entry.unit">{{ entry.unit }}</code>
        <pre>{{ entry.message }}</pre>
      </div>
      <div v-if="!loading && !entries.length" class="log-empty">当前筛选条件下没有日志记录</div>
    </section>
  </div>
</template>

<style scoped>
.log-toolbar{display:flex;min-height:64px;align-items:center;justify-content:space-between;gap:14px;padding:10px 12px}.log-filters,.log-tools{display:flex;align-items:center;gap:8px}.log-filters :deep(.el-select){width:155px}.log-tools :deep(.el-input){width:min(340px,28vw)}.log-toolbar :deep(.el-select__wrapper),.log-toolbar :deep(.el-input__wrapper){min-height:40px;border-radius:9px}.follow-switch{display:flex;min-height:40px;align-items:center;gap:7px;padding:0 10px;border:1px solid var(--ncp-line);border-radius:9px;color:var(--ncp-text-muted);font-size:.76rem}.log-console{overflow:hidden}.log-head,.log-row{display:grid;grid-template-columns:178px 72px 190px minmax(360px,1fr);align-items:start;gap:12px}.log-head{min-height:42px;align-items:center;padding:0 14px;background:var(--ncp-surface-quiet);color:var(--ncp-text-subtle);font-size:.72rem;font-weight:720}.log-row{min-height:48px;padding:9px 14px;border-top:1px solid var(--ncp-line);font-size:.72rem}.log-row:hover{background:var(--ncp-surface-hover)}.log-row time,.log-row code{overflow:hidden;color:var(--ncp-text-subtle);font-family:var(--ncp-font-mono);font-size:.67rem;text-overflow:ellipsis;white-space:nowrap}.log-row pre{margin:0;overflow-wrap:anywhere;color:#33435b;font-family:var(--ncp-font-mono);font-size:.69rem;line-height:1.55;white-space:pre-wrap}.level-badge{justify-self:start;padding:3px 7px;border-radius:6px;background:var(--ncp-info-soft);color:var(--ncp-info);font-size:.66rem;font-weight:720}.level-badge--error{background:var(--ncp-danger-soft);color:var(--ncp-danger-strong)}.level-badge--warning{background:var(--ncp-warning-soft);color:var(--ncp-warning-strong)}.level-badge--debug{background:var(--ncp-surface-quiet);color:var(--ncp-text-subtle)}.log-row--skeleton i{width:75%;height:10px}.log-empty{display:grid;min-height:220px;place-items:center;color:var(--ncp-text-subtle);font-size:.8rem}.log-error{padding:11px 14px;border-radius:9px;background:var(--ncp-danger-soft);color:var(--ncp-danger-strong);font-size:.78rem}@media(max-width:1050px){.log-toolbar{align-items:stretch;flex-direction:column}.log-filters,.log-tools{width:100%}.log-filters :deep(.el-select){flex:1}.log-tools :deep(.el-input){min-width:0;flex:1}.log-head,.log-row{grid-template-columns:155px 64px 150px minmax(280px,1fr)}}@media(max-width:700px){.log-filters{display:grid;grid-template-columns:1fr 1fr}.log-filters :deep(.el-select){width:100%}.log-head{display:none}.log-row{grid-template-columns:1fr auto}.log-row code{grid-column:1}.log-row pre{grid-column:1/-1}.log-row time{grid-row:1;grid-column:1}}
</style>
