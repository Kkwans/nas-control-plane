...
import { KeyRound, MoreHorizontal, Plus, RefreshCw, ShieldCheck, UserRound, UserRoundCheck, UserRoundX } from '@lucide/vue'
...
async function submitCreate() {
  if (!createForm.value.username.trim() || createForm.value.password.length < 8) {
    ElMessage.warning('请输入用户名和至少 8 个字符的密码。')
    return
  }
  saving.value = true
  try {
...
async function submitPassword() {
  ...
  if (passwordForm.value.newPassword.length < 8 || passwordForm.value.newPassword !== passwordForm.value.confirmation) {
    ElMessage.warning('新密码至少 8 个字符，且两次输入必须一致。')
    return
  }
  saving.value = true
...
<template>
  <div class="page workspace-page users-page">
    <WorkspaceHeader title="用户管理" description="管理可登录 NCP 的等权 Root 账号与会话安全" :icon="UserRound" :stats="stats">
      <template #actions>
        <ElButton :icon="RefreshCw" :loading="loading" @click="loadUsers">刷新</ElButton>
    ...
    <ElDialog v-model="createOpen" title="新增 Root 账号" width="440px" destroy-on-close>
      <div class="dialog-form">
        ...
        <label><span>初始密码</span><ElInput v-model="createForm.password" type="password" show-password autocomplete="new-password" placeholder="至少 8 个字符" /></label>
      </div>
    ...
    <ElDialog v-model="passwordOpen" :title="selectedUser?.id === currentUserId ? '修改自己的密码' : `重置 ${selectedUser?.username} 的密码`" width="440px" destroy-on-close>
      <div class="dialog-form">
        ...
        <label><span>新密码</span><ElInput v-model="passwordForm.newPassword" type="password" show-password autocomplete="new-password" placeholder="至少 8 个字符" /></label>
